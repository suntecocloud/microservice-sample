...
export const authenticate = async function (req: Request, next) {
    try {
        ...
        const authenticationUrl: string = `http://${ContextPathMap.get('id')}/authenticate`
        console.log('---------------')
        console.log(authenticationUrl)
        try {
            const response = await axios.post(authenticationUrl, {jwtToken: jwtToken},{validateStatus:function (status) {
